"""Licitações-e (Banco do Brasil) — pregões de municípios via scraping HTML."""
import logging
import re
...
SEARCH_URL = "https://www.licitacoes-e.com.br/aop/pesquisar-licitacao.aop"
...
def _parse_date_be(val: Optional[str]) -> Optional[date]:
    if not val:
        return None
    val = val.strip()
...
def _parse_row(cells) -> Optional[dict]:
    """Converte linha da tabela de resultados em dict de bid."""
    try:
        texts = [c.get_text(strip=True) for c in cells]
        if len(texts) < 5:
            return None

        # Colunas típicas: Número | Órgão | Objeto | Abertura | Encerramento | Valor
        external_id = texts[0].strip()
        if not external_id:
            return None

        closing = _parse_date_be(texts[4] if len(texts) > 4 else None)
        today = date.today()
        ...
        return {
            ...
            "opening_date": _parse_date_be(texts[3] if len(texts) > 3 else None),
...
async def sync_licitacoes_e(days_back: int = 1):
    start_time = datetime.utcnow()
    log = ScrapeLog(source="licitacoes_e", start_time=start_time)
    ...
    try:
            ...
            while True:
                try:
                    resp = await client.post(SEARCH_URL, data={
                        ...
                        "situacaoLicitacao": "A",   # Abertas
                        "pagina": str(page),
                        "itensPorPagina": "100",
                    }, headers=HEADERS)
                    resp.raise_for_status()
                except Exception as e:
                    logger.warning(f"Licitações-e pág {page}: {e}")
                    break
        ...
        logger.exception("Erro ao sincronizar Licitações-e")
    ...
    await update_source_status("licitacoes_e", log.status, inserted + updated)
    logger.info(f"Licitações-e sync: found={found} inserted={inserted} updated={updated} status={log.status}")
    return log
